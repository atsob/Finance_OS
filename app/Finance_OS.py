...
# Χρησιμοποιούμε impersonate για να φαινόμαστε σαν Chrome σε Windows
custom_session = cryq.Session(verify=False, impersonate="chrome110")
...
db = SQLDatabase.from_uri(pg_uri, 
    include_tables=['accounts', 'bank_transactions', 'bank_transaction_splits', 'categories', 'currencies','financialinstitutions', 'historical_fx', 'historical_prices', 'holdings', 'investment_transactions', 'payees', 'securities'], 
    sample_rows_in_table_info=30
)

# ΠΡΟΣΘΕΣΕ ΑΥΤΕΣ ΤΙΣ ΓΡΑΜΜΕΣ ΕΔΩ:
#print("--- ΕΛΕΓΧΟΣ ΒΑΣΗΣ ΔΕΔΟΜΕΝΩΝ ---")
#actual_tables = db.get_usable_table_names()
#print(f"Πίνακες που βρέθηκαν: {actual_tables}")
#st.sidebar.write(f"Συνδεδεμένοι πίνακες: {actual_tables}") # Για να το βλέπεις και στο UI


# 2. Αρχικοποίηση του LLM (Ollama - fully local)
ollama_ip = os.getenv("OLLAMA_IP", "127.0.0.1")
ollama_port = os.getenv("OLLAMA_PORT", "11434")
ollama_model = os.getenv("OLLAMA_MODEL", "llama3.2:3b")

# Κατασκευή του base_url
...
# ─── AI Assistant router ───────────────────────────────────────────────────
def run_ai_assistant(user_input: str) -> str:
    """
    Routes the question to:
      - SQL agent  (database questions)
      - Web search + LLM  (market/news questions)
    ...
    # Step 1: ask the LLM how to route
    decision = llm.invoke(
        f"Today is {full_date}. Classify this question into exactly one category:\n"
        f"- SQL   → needs data from a personal finance database (accounts, transactions, holdings, prices)\n"
    ...
    if "SQL" in decision:
        ...
        config = {"configurable": {"session_id": "finance_os"}, "callbacks": [st_callback]}
        response = agent_with_chat_history.invoke({"input": full_prompt}, config=config)
...
#                format_func=lambda x: x['accounts_name']
    ...
    # --- ⚙️ MARKET DATA ---
        ...
        st.caption(f"Powered by {ollama_model} · Fully local · Your data stays on your Pi")
        ...
        if user_input:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    answer = run_ai_assistant(user_input)
                st.write(answer)
            # Persist user message in history (assistant reply is saved by the agent)
